...
:: Needs MathLibrary.cmd code to handle large numbers

SET _PCount=1
SET _PCheck=1
SET _FindFactor=600851475143
SET _FactorCheck=
SET _LargeFactor=
SET _CompareRes=
...
:: Find a prime number...
SET /A _PCheck+=2
SET /A _PCheckMax=%_PCheck%/3
FOR /L %%G IN (1,1,10) DO (
	IF DEFINED _PHash%%G (
		FOR %%H IN (!_PHash%%G!) DO (
			IF %%H LEQ %_PCheckMax% (
				SET /A _PrCheck = !_PCheck! %% %%H
				IF !_PrCheck! EQU 0 GOTO PLoop
			)
		)
	) 
)
...
FOR /L %%I IN (1,1,10) DO (
	IF %_PCount% LSS %%I000 (
		IF DEFINED _PHash%%I (
			SET _PHash%%I=!_PHash%%I!,%_PCheck%
			GOTO PBreak
		) ELSE (
			SET _PHash%%I=%_PCheck%
			GOTO PBreak
		)
	)
)
...
:ExtDivision
	...
	IF %_DivNLen% GEQ %_DivDLen% (
		SET /A _DivMaxPrec=_DivNLen+1
	) ELSE (
		SET /A _DivMaxPrec=_DivDLen+1
	)
